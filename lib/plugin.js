...
export default async function({ app, $config }) {
    config.baseURL = $config.hatchly.navigation.browserBaseURL || $config.hatchly.navigation.baseURL;

    config.apiURL = process.server
        ? $config.hatchly.navigation.baseURL
        : config.baseURL;

    config.apiURL = `${ config.apiURL }/${ $config.hatchly.navigation.apiPath }`;
