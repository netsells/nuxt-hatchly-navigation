...
const urlFormatter = (url) => {
    const [protocol, _, hostname] = url.split('/');
...
export const registerStoreModule = (store) => {
    store.registerModule('navigations', {
        ...
        actions: {
             ...
             */
            async get({ commit }) {
                const groupsPath = `${ config.endpoint }/groups`;
                ...
                const requests = data.map(({ key }) => {
                    const groupPath = `${ config.endpoint }/groups/${ key }/items`;
    ...
    }, { preserveState: Boolean(store.state['navigations']) });
};
...
export default async function({ app, $config, route }, inject) {
    config.endpoint = $config.hatchly.navigation.endpoint;

    registerStoreModule(app.store);

    inject('nav', globalMixin);

    if (app.store.state && app.store.state.navigations && app.store.state.navigations.initialised && !Boolean(app.previewData)) {
        return;
    }
