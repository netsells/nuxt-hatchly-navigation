...
const urlFormatter = (url) => {
    return (url || '')
        ...
        .replace(config.baseURL || '<%- options.baseUrl %>', '')
...
export default async function({ app, $config }) {
    config = {
        ...
        ...$config.axios,
        ...$config.hatchly,
        ...($config.hatchly && $config.hatchly.navigation),
    };

    config.apiUrl = process.client
        ? process.env.API_URL || config.baseURL || '<%- options.baseURL %>'
        : process.env.API_URL_BROWSER || config.browserBaseURL || '<%- options.browserBaseURL %>';

    config.apiUrl = `${ config.apiUrl }/<%- options.apiPath %>`;
